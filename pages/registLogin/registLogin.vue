<template>
	<view class="body">
		<from @submit="formSubmit">
			...
			<button type="primary" from-type="submit" style="margin-top: 60upx;width: 90%;">注册/登录</button>
		</from>
					...
					success(loginRes) {
						...
						uni.getUserInfo({
							...
							success(info) {
								...
								// 调用开发者后台，执行一键注册或登录
								uni.request({
									url: _this.serverUrl + '/appUnionLogin/' + logintype,
									data: {
			...
			wxLogin(e) {
				...
				uni.login({
					...
					success(loginResult) {
						...
						// 设置登录到哪个对于的微信小程序，可以根据自己的后端去实现业务参数
